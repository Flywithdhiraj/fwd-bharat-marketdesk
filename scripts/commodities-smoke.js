...
assert(commodity.includes('Use Spread Desk for calendar and same-expiry size-matched structures.'));
assert(commodity.includes('>Market</button>') && commodity.includes('>Spreads</button>') && commodity.includes('>Research</button>'));
assert(commodity.includes('Manual two-leg ticket') && commodity.includes('Historical evidence'));
assert(commodity.includes('commodity-spread-evidence.json') && commodity.includes("grade === 'SUPPORTED'"));
assert(commodity.includes('Same-expiry size matched') && commodity.includes('Estimate Combined Margin'));
assert(commodity.includes('data-spread-manual-direction') && commodity.includes('saveManualSpreadPlan'));
assert(!commodity.includes('GOLD / GOLDM matched') && !commodity.includes('function spreadResearchHtml'));
...
assert(commodity.includes('function patchCommodityLiveValues') && commodity.includes('patchCommodityLiveValues();'));
assert(commodity.includes('data-commodity-live-symbol') && commodity.includes('data-spread-live-field'));
...
assert(css.includes('.commodity-spread-manual-ticket') && css.includes('.commodity-spread-trade-read button'));
assert(css.includes('.commodity-spread-evidence') && css.includes('.commodity-spread-evidence.supported'));
...
assert(service.includes("['GOLD', 'GOLDM']") && service.includes("['SILVER', 'SILVERM', 'SILVERMIC']") && service.includes("['CRUDEOIL', 'CRUDEOILM']") && service.includes("['NATURALGAS', 'NATGASMINI']"));
...
assert(service.includes('filterMatchedCommodityRows') && service.includes('relativeGap <= 0.03'));
assert(chartEngine.includes("contracts.pairType === 'matched'") && chartEngine.includes('contracts.near?.lots'));
assert(commodity.includes('commodity-spread-exact-plan') && commodity.includes('Buy spread = second ask - first bid'));
...
assert(service.includes('cleaningVersion: 4') && service.includes('outdatedCleaning'));
