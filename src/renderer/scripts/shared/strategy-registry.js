...
(function initStrategyRegistry(global) {
 ...
 canLiveTrade: false,
  ...
  pullback: Object.freeze({
   ...
   shortName: 'Pullback',
   description: 'Scanner-only trend pullback lab for 9 EMA touch, undercut/reclaim, round-number support, OBV confirmation, and late-entry rejection.',
   mode: STRATEGY_MODES.SCANNER_ONLY,
   market: 'Delta perpetuals',
  ...
  native_straddle: Object.freeze({
   id: 'native_straddle',
   displayName: 'Native Straddle Scanner',
   shortName: 'Native Straddle',
   description: 'Scanner-only MV native straddle ideas for BTC and ETH using 15m chart review, live premium, spread, liquidity, and market-regime scoring.',
   mode: STRATEGY_MODES.SCANNER_ONLY,
   market: 'Delta MV native straddles',
   timeframe: '15m chart handoff',
   resultKey: 'strategyResults.native_straddle',
   statusKey: 'strategyStatus.native_straddle',
   settingsKey: 'strategySettings.native_straddle',
   scannerAction: 'native-straddle:startScan',
   canLiveTrade: false,
   canPaperTrade: false,
   summaryFields: Object.freeze(['eventType', 'actionLabel', 'priorityLabel', 'score', 'entry', 'stop']),
  }),
