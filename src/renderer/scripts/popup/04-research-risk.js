...
function focusFundingSymbolInScanner(symbol) {
 ...
 globalThis.scheduleWorkspaceTabRender?.('scanner') || renderScanner();
...
document.getElementById('btnCSV')?.addEventListener('click', () => {
 ...
 a.download = `fwd_tradedesk_pro_report_${new Date().toISOString().slice(0, 10)}.csv`;
...
document.getElementById('btnScan')?.addEventListener('click', async () => {
 ...
 chrome.runtime.sendMessage({ action: 'native-straddle:startScan' }, () => {
  if (chrome.runtime.lastError) {
   console.warn('Native straddle scan skipped:', chrome.runtime.lastError.message || chrome.runtime.lastError);
  }
 });
